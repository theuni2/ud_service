...
export default function PricingUSA() {
  return (
    <section
      id="pricing-usa"
      className="relative bg-gradient-to-br from-indigo-50 via-white to-purple-50 py-20 px-4 sm:px-8 lg:px-16"
    >
      {/* Header */}
      <div className="max-w-6xl mx-auto text-center mb-20">
        <motion.h2
        ...
        <motion.p
          ...
          className="mt-4 max-w-3xl mx-auto text-base sm:text-lg text-gray-700"
        >
          A comprehensive guidance program — from personalized admissions
          strategy to complete profile building and execution support.
        </motion.p>
      </div>
      ...
      <div className="max-w-7xl mx-auto grid gap-10 lg:grid-cols-2">
        ...
        <div className="space-y-8">
          ...
          >
            <div className="flex items-center gap-3 mb-5">
              <GraduationCap className="h-8 w-8 text-indigo-600" />
              <h3 className="text-2xl font-semibold text-gray-900">Coverage</h3>
            </div>
            <ul className="space-y-3 text-gray-700 text-base">
              <li>
                <CheckCircle className="h-5 w-5 text-indigo-600 inline mr-2" />
                10–12 colleges across 2–3 countries
              </li>
              <li>
                <CheckCircle className="h-5 w-5 text-indigo-600 inline mr-2" />
                Example: 7–8 USA, 2–3 AU, 1 UCAS (5 colleges)
              </li>
              <li>
                <CheckCircle className="h-5 w-5 text-indigo-600 inline mr-2" />
                Extracurriculars: 5–8 activities covered
              </li>
            </ul>
          </motion.div>
          ...
          >
            ...
            <ul className="grid sm:grid-cols-2 gap-x-6 gap-y-3 text-gray-700 text-base">
              {[
                "Assessment",
                "Shortlisting",
                ...
                <li key={i}>
                  <CheckCircle className="h-4 w-4 text-indigo-600 inline mr-2" />
                  {item}
                </li>
              ))}
            </ul>

            <h4 className="mt-8 mb-3 text-lg font-semibold text-gray-900 flex items-center gap-2">
              <Briefcase className="h-5 w-5 text-indigo-600" /> Profile Building
            </h4>
            <ul className="grid sm:grid-cols-2 gap-x-6 gap-y-3 text-gray-700 text-base">
              {[
                "1 Research Project",
        ...
        {/* Right Section – What We Cover & Don’t Cover */}
        <div className="space-y-8">
          {/* What We Cover */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="bg-gradient-to-tr from-green-50 to-white rounded-3xl shadow-lg p-8 border border-green-100 hover:shadow-xl transition-all duration-300"
          >
            ...
            <ul className="space-y-3 text-gray-700 text-base leading-relaxed">
              {[
                ...
                "Competition & Publication Recommendations",
              ].map((item, i) => (
                <li key={i}>
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ delay: 0.2, duration: 0.6 }}
            className="bg-gradient-to-tr from-red-50 to-white rounded-3xl shadow-lg p-8 border border-red-100 hover:shadow-xl transition-all duration-300"
          >
            <h3 className="text-2xl font-semibold text-gray-900 mb-5 flex items-center gap-2">
              ❌ What We Don’t Cover
            </h3>
            <ul className="space-y-3 text-gray-700 text-base leading-relaxed">
              {[
                "Publisher costs for papers/books/websites",
                "Prototype procurement costs (robots, devices)",
                "School tuitions (SAT, AP, IELTS, IB, etc.)",
                "College application or summer school fees",
                "VISA costs",
              ].map((item, i) => (
                <li key={i}>
      ...
      >
        <a
          ...
          className="inline-block bg-indigo-600 text-white text-lg font-semibold py-3 px-8 rounded-xl shadow-lg hover:bg-indigo-700 transition"
        >
